...
| NARA file-unit quality | Open | Kept as a visible risk; expanded Scout query packs should be rerun after API quota recovery |
...
- The NARA Scout generated data still contains cached, undated file-unit rows from the earlier API-limited run.

## Next pull list

1. Re-run the NARA Scout harvester with the expanded query packs once `NARA_SCOUT_API_KEY` quota is available.
2. Screen `2016-0048-M`, `2014-0644-M`, `2016-0158-F`, `2013-0870-M`, `2013-0865-M`, and `2015-0641-M` for item-level documents.
3. Replace source-path-only leads for China, South Asia, and the DCI threat-assessment lane with official item pages or stronger primary sources.
4. Add State/ACDA/DOE/JCS/CIA source records where available, especially for CTBT zero-yield, CWC ratification, HEU transparency, theater missile defense, and BWC protocol negotiations.
